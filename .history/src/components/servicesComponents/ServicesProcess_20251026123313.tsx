      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <motion.div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6" variants={staggerContainer} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }}>
          {processSteps.map((step: any, index: number) => (
            <motion.div key={index} variants={scaleIn} className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-lg relative">
              <div className="text-6xl font-bold text-teal-100 dark:text-gray-700 absolute top-4 right-4 opacity-50">{step.number}</div>
