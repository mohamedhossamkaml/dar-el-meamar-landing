...
const Hero = ({ scrollToSection, withVanta }: Props) => {
  ...
  const vantaColor = theme === "dark" ? 0x14b8a6 : 0xff6f61;
  const vantaBg = theme === "dark" ? 0x0f172a : 0xfefefe;

  return (
<motion.section id="home" variants={staggerContainer} initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.2 }} className="relative pt-20 min-h-screen flex items-center overflow-hidden" >
      {/* Vanta Background */}
      {withVanta && (
        <VantaBackground
          enabled={true}
      ...
      <div className="absolute inset-0 -z-10 bg-gradient-to-br from-slate-50 via-white to-teal-50
                      dark:from-gray-900 dark:via-gray-900 dark:to-gray-800 opacity-80" />
