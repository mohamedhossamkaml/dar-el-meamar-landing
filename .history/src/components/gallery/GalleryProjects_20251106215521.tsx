...
const GalleryProjects: React.FC = () => {
  ...
  const projects = t.galleryPage.projects.map((proj: any) => ({
    ...proj,
    images: PROJECTS_EXTRA[proj.id]?.images || [], // يضيف الصور من الـ config حسب الـ id
    color: PROJECTS_EXTRA[proj.id]?.color || 'from-gray-400 to-gray-600', // يضيف اللون من الـ config أو لون افتراضي
  }));
