...
const WorksCapabilities: React.FC = () => {
  ...
  // Extract capabilities from translation object
  const capabilitiesFromT = Object.values(t.worksPage?.capabilities || {}) as {
    title: string;
    description: string;
  }[];

  // Map capabilities to include corresponding icons
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          {capabilitiesWithIcons.map((capability, index) => (
            ...
            >
              <capability.icon
                className="text-teal-600 dark:text-teal-400 mx-auto mb-4 transition-transform duration-300 hover:rotate-6"
                size={56}
              />
