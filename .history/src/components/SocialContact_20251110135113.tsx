...
const SocialContact: React.FC<SocialContactProps> = ({ position = "left" }) => {
  ...
  return (
    ...
    >
      ...
      {/* Label */}
      {open && (
        <span className="text-xs text-gray-600 dark:text-gray-300 mt-1 font-semibold animate-fadeIn">
          معلومات التواصل
        </span>
      )}
      ...
      >
        {icons.map((item, index) => (
          <a
            ...
            className={`${item.bg} text-white p-3 rounded-full shadow-md transition transform ${open
              ? `delay-[${index * 100}ms] scale-100`
              : "scale-0"
              }`}
          >
            {item.icon}
