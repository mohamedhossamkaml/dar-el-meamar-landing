...
const ProjectPage: React.FC = () => {
  ...
  const projectData = useMemo(() => {
    ...
    // if no project found id is not a number or out of bounds
    const projectsFromT = Object.values(t.galleryPage?.projects || {}) as {
      id: number;
      title: string;
      description: string;
      category?: string;
    ...
    // if no project found id is not a number or out of bounds
    const projFromT = projectsFromT.find((p) => Number(p.id) === idx);
    ...
    const grouped = Object.entries(detailsByCategory).map(([category, imgs]) => ({
      category,
      imgs,
    }));
        ...
        <div className="max-w-5xl mx-auto px-4">
          ...
          {projectData.grouped.length > 0 ? (
            ...
            <p className="text-gray-500">
              {t.projectPage?.noImages || 'No images available'}
            </p>
          )}
          ...
          <div className="mt-8">
            <SeeMoreButton
              to="/contact"
              label={t.galleryPage?.cta?.button ?? 'Contact'}
            />
          </div>
        </div>
      </motion.main>
